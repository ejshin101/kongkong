...
public class ChattingHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		List<FlightVO> chat = service.getChat();
		for(FlightVO text : chat){
			session.sendMessage(new TextMessage(text.getChatId()+"-"+text.getText()+"-"+text.getChatDate().substring(11,16) ));
		}
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        for(WebSocketSession sess : sessionList){
            sess.sendMessage(new TextMessage(user.getUserName()+"-"+message.getPayload()+"-"+date.format(nowDate)));
        }
