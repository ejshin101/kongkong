...
public class MailSendService {
	...
	public String getKey(int size) {
	 ...
	 public String sendStringMail(String memMail, String memId) {

	        MimeMessage mail = mailSender.createMimeMessage();
	        String mailContent = "귀하의 " + memId + " 번 게시물이 부적절한 표현이나 기타 사유로 인해 블락처리 되었음을 알려드립니다. " ;     //보낼 메시지 
	        try {
	        	System.out.println("sdfsdf");
	            mail.setSubject(" 게시글 차단 안내 ", "utf-8"); // 
	            mail.setText(mailContent, "utf-8", "html");   //보낼 메시지
	            mail.addRecipient(Message.RecipientType.TO, new InternetAddress(memMail));
	            mailSender.send(mail);
	        } catch (MessagingException e) {
