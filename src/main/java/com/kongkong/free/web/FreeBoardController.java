...
//		req.setAttribute("boardScope", boardScope);
	...
	public String mileageShopView(HttpSession session, Model model) {
		UserVO user = (UserVO)session.getAttribute("USER_INFO");
		if(user != null) {
	...
	public String getNewOrderCode() {
		...
		return "MERCHANT_MILE_" +  UUID.randomUUID().toString().substring(0,6) ;  // MERCHANT_ASEDE
	}
	
	@RequestMapping("/free/insertMileage.wow")
	public String insertMileage(HttpSession session, @ModelAttribute("reserve") ReservationVO reserve) {
		UserVO user = (UserVO)session.getAttribute("USER_INFO");
		if(user != null && user.getUserMileage() >= 520000) {
			
			MemberVO member = memberService.getMember(user.getUserId());
			
			// reserve.setBookBuyerAddr(member.getMemAdd1(), member.getMemAdd2());
			reserve.setBookId(member.getMemId());
			reserve.setBookMerchantUid(getNewOrderCode());
			
			freeBoardService.insertMileage(reserve);
			freeBoardService.insertMileage2(reserve);
