...
<!-- START : 검색 폼  --> 
	...
	<form name="frm_search" action="boardHome.wow" method="post">
			<input type="hidden" name="curPage" value="${searchVO.curPage}">
	        <input type="hidden" name="rowSizePerPage" value="${searchVO.rowSizePerPage}">
			<div style="margin: 5px;"></div>
			<div class="row">
				<div class="col-md-10" style="padding: 0px;">
					<%@ include file="/WEB-INF/inc/topFreeBoard.jsp"%>
				</div>
			...
			<div class="row" style="margin-top: -10;">
				...
				padding-top: 20px; padding-left: 30px; padding-right: 30px;">
					...
					<c:forEach items="${onlyFreeBest}" var="best" >
					<div class="col-4 border-right"
						      ...
						      <style>
						      	.cls_best_image img {padding: 10px; width: 380px; height: 260px;}
						      </style>
						      <span class="cls_best_image">
							      <c:if test="${not empty best.boImg }">
							      	<a href="freeView.wow?boNo=${best.boNo}">${best.boImg}</a>
							      </c:if>
							      <c:if test="${empty best.boImg }">
							      	<img src="<%=request.getContextPath()%>/resources/image/nullimg.png" />
							      </c:if>
						      </span>
							  <div class="text-center">
							  <c:if test="${best.boHit >= 100}">
							  <span class="badge badge-primary">Best of kongkong</span>
							  </c:if>
							      	<h4><a href="freeView.wow?boNo=${best.boNo}">${best.boTitle}</a></h4>
							      	<p style="text-align: right;">${best.boWriter} / ${best.boRegDate}</p>
							      </div>
						      </div>
						</c:forEach>	      
	...
	<!-- 공지게시판 끝 -->
	<!-- 메인게시판 시작 -->
					<c:forEach items="${freeBoardList}" var="freeBoard" varStatus="st">
						<tr class="">
							<%-- <td>${freeBoardList.size()-st.count+1}  </td> --%>
							<td>${freeBoard.boNo}</td>
							<td>${freeBoard.boCategory}</td>
							<td class="text-left">
							<c:if test="${freeBoard.boBlock eq 'Y'}">
								<b style="color: red;">관리자에 의해 차단된 게시글 입니다.</b>
							</c:if>
							<c:if test="${freeBoard.boBlock eq 'N'}">
								<a href="freeView.wow?boNo=${freeBoard.boNo}">
									<c:if test="${freeBoard.boHit >= 100}"><span class="badge badge-primary">BOK</span></c:if> ${freeBoard.boTitle}  
									<c:if test="${freeBoard.boReplyCnt ge 1}"><span class="text-primary" style="font-size: small;">[${freeBoard.boReplyCnt}]</span></c:if>
								</a>
							</c:if>
							</td>
							<td>${freeBoard.boWriter}<c:if test='${sessionScope.USER_INFO.userId eq freeBoard.boId}'><a href="/myPage/myPageHome.wow"><i class="fas fa-user-circle"></i></a></c:if></td>
