    ...
    <form name="frm_search" action="reservationList" method="get" class="form-horizontal">
        ...
        <input type="hidden" name="rowSizePerPage" value="${searchVO.rowSizePerPage }">
          <div class="form-group row">
            ...
            <div class="col-sm-5">
	            <input type="radio" name="searchType" value="DT01"> 수령일
	            <input type="radio" name="searchType" value="DT02" > 반납일
	            <input type="radio" name="searchType" value="DT03" > 결제일
	            <input type="radio" name="searchType" value=""  checked> 선택안함
            </div>
          ...
          <div class="form-group row">
          <label for="id_searchShipping" class="control-label"></label>
          <div class="col-sm-5">
	            <input type="radio" name="searchShipping" value="SH01"> 수령
	            <input type="radio" name="searchShipping" value="SH02" > 발송
	            <input type="radio" name="searchShipping" value="SH03" > 반납
	            <input type="radio" name="searchShipping" value="SH04" > 구매취소
	            <input type="radio" name="searchShipping" value=""  checked> 선택안함
            </div>
            <div class="col-sm-1">
            Y/N
            </div>
            <div class="col-sm-1">
            	<select id="id_searchYN" name="searchYN" class="form-control input-sm">
            		<option value="">---</option>
...
<!-- END : 목록건수 및 새글쓰기 버튼  --> 
	<table class="table table-bordered table-hover text-center fifth-gray" style="margin-bottom: 50px">
	<thead>
		<tr>
			<th>주문 번호</th>
			<th>품목</th>
...
<tr>
		...
		<td>${vo.bookAmount}</td>
		<td>${vo.bookDate }</td>
		<td>${vo.bookPickupYn }</td>
		<td>${vo.bookShippingYn }</td>
		<td>${vo.bookReturnYn }</td>
		<td>${vo.bookDel }</td>
	 </tr>
	 </c:forEach>
	</tbody>
...
$('#id_btn_reset').click(function(e) {
	...
	$frm.find("input[name=searchType]").val("").prop('checked',true);
	$frm.find("input[name=searchShipping]:radio[value='']").prop('checked',true);
	$frm.find("select[name=searchYN]").val("");
