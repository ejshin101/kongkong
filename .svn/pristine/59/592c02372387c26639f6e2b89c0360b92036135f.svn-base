...
public class AdminController {
	...
	public String adminPageManage( Model model) {
		// 총회원수 조회
		MemberVO member = (MemberVO) adminService.getMemberCount();
	...
	// 신고관리
		...
		public String realSingo(Model model, FreeBoardVO free, @ModelAttribute("searchVO") SingoBlockSearchVO searchVO) {
			
			searchVO.setTotalRowCount(adminService.getSingoCount(searchVO));
			searchVO.setting();
			
			List<FreeBoardVO> singoInfo = adminService.freeSelect(searchVO);
		...
		public String block(Model model, FreeBoardVO free, MemberVO member,  @ModelAttribute("searchVO") SingoBlockSearchVO searchVO) {
			
			searchVO.setTotalRowCount(adminService.getBlockCount(searchVO));
			searchVO.setting();
			
			free.setBoBlockMail(member.getMemMail());
			System.out.println(member.getMemMail());
			List<FreeBoardVO> blockInfo = adminService.freeBlockSelect(searchVO);
