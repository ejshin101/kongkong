...
<div class="container">
	<table class="table">
				...
				<tr>
					...
					<div class="row">
						...
						<h2><c:if test="${boards.boHit >= 100 && boards.boCategory != 'NOTICE'}"><a href="#" onclick="message();"><span class="badge badge-primary">Best of kongkong</span></a>
							</c:if>[${boards.boNo}] ${boards.boTitle}</h2>
						<input type="hidden" value="${boards.boNo}">
						</div>
						<div class="col-md-1" style="text-align: right;">
							<a class="btn btn-warning btn-sm cls-favorite-change " data-fa-bo-no="${boards.boNo}"  data-fa-category="FREE" data-fa-mem-id="${sessionScope.USER_INFO.userId}" title="즐겨찾기">
		...
		<div class="row">
			...
			<c:if test="${boards.boCategory eq 'FREE' }">
              <a href="boardHome.wow" class="kongkongs">
			...
			<c:if test="${boards.boCategory eq 'NOTICE' }">
              <a href="boardNotice.wow" class="kongkongs">
                <span class="glyphicon glyphicon-save" aria-hidden="true"></span>
                목록
              </a>
			</c:if>
			
			<c:if test="${boards.boCategory eq 'TIP' }">
              <a href="boardTip.wow" class="kongkongs">
                <span class="glyphicon glyphicon-save" aria-hidden="true"></span>
                목록
              </a>
			</c:if>
...
<div class="container">
	...
	<!-- START : 댓글 목록 영역 -->
		...
		<div class="row text-center justify-content-center" id="id_reply_list_more">
			<button id="btn_reply_list_more" class="kongkongs">
			...
			success : function (data) {
				...
				$.each(data.data,function(i,el){
					...
					var str = '<div class="row" >';
				        ...
				        str += '<div class="col-sm-5"><label>'+el.reContent+'</label></div>';
...
//--- 3.이벤트함수 선언
	...
	$(document).ready( function() {
				...
				// 4. 초기화 함수 호출
				fn_reply_list();
				fn_favority_get('FREE',${boards.boNo});
