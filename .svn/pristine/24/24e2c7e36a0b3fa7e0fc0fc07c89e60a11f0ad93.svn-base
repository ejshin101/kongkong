...
<%@ attribute name="pagingVO" type="com.study.common.vo.PagingVO" required="true"%>
<%--UI 공통적으로 사용되는것 중 --%>
<!-- //start .pagination -->
<%
	//절대경로인 경우 앞에 컨택스트패스 추가 
...
<nav class="text-center">
  ...
  <c:if test="${pagingVO.startPage > 1}">
    <li>
      <a href="${linkPage}?curPage=${pagingVO.startPage - 1}" 
    ...
    <c:forEach begin="${pagingVO.startPage}" end="${pagingVO.endPage}" var="i">
    	<!-- 만약 i하고 curPage가 같은면 -->
    	<li ${pagingVO.curPage eq i ? 'class="active" ': ''} ><a href="freeList.wow?curPage=${i}" 
    		data-page="${i}">${i}</a></li>
    </c:forEach>
    
   <c:if test="${pagingVO.endPage < pagingVO.totalPageCount}">
    <li>
      <a href="${linkPage}?curPage=${pagingVO.endPage+1}" 
