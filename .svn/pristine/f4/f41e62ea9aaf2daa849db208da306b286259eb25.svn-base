...
<!-- START : 검색 폼  --> 
    ...
    <form id="frm_search" name="frm_search" action="Block.wow" method="get" class="form-horizontal" onsubmit="return false;">
...
<table class="table text-center">
				...
				<c:forEach items="${blockInfo}" var="blockInfo" varStatus="st">
					<tr class="blocklist">
						...
						<td>
							<%-- <a href="blockBack.wow?boNo=${blockInfo.boNo}" style="margin-right: 10px;" class="kongkongsRed-sm">블락 취소</a>
							<a href="blockDelete.wow?boNo=${blockInfo.boNo}" class="kongkongsYellow-sm">글 삭제</a> --%>
							<button class="kongkongsRed-sm"  style="margin-right: 10px;" data-bo-no="${blockInfo.boNo}" type="button" >블락 취소</button>
							<button class="kongkongsYellow-sm"  style="margin-right: 10px;" data-bo-no="${blockInfo.boNo}" type="button" >글 삭제</button>
						</td>	
						<td><button onclick="fn_email('${blockInfo.boBlockMail}','${blockInfo.boNo}')" id="memMail" style="margin-right: 10px;" class="kongkongs-sm">메일전송</button>
						</td>
					</tr>
					</c:forEach>
				</tbody>
			</table> 
		<kongkong:pagingBlock pagingVO="${searchVO}" />
</body><!-- end.container -->
...
function fnSubmit() {
	// $('#targetdiv').load('Block.wow', $("#frm_search").serialize(), "post");
	$.ajax({
		url: "block.wow",		
